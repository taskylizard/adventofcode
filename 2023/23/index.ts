...
function getGraph(input: string, { noUphill = false } = {}) {
    const board = input
        ...
        .filter(Boolean)
        .map((line, row) =>
            line.split("").map<Path>((type, col) => ({ row, col, type, neighbors: [] })),
        );
...
function getRouteTotalLength<T>(path: Node<T>[]): number {
    return path
        .map((node, i) =>
            i < 1 ? 0 : node.neighbors.find(({ destination }) => destination === path[i - 1])!.distance,
        )
        .reduce((a, b) => a + b);
}
...
run({
    part1(context) {
        return solvePart1(context.input);
    },
    part2(context) {
        return solvePart2(context.input);
    },
});
